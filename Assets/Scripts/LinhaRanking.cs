...
/// ├── TextoFase3     ← TMP_Text  ex: "00:58.33"
/// └── TextoTotal     ← TMP_Text  ex: "04:31.78"
/// </summary>
public class LinhaRanking : MonoBehaviour
{
    ...
    // ── API pública ────────────────────────────────────────
    public void Preencher(int posicao, string nome, float fase1, float fase2, float fase3, float total)
    {
        ...
        if (textoFase3   != null) textoFase3.text   = Formatar(fase3);
        if (textoTotal   != null) textoTotal.text   = Formatar(total);

        // Cor de fundo conforme posição
        if (fundoLinha != null)
        {
            switch (posicao)
            {
